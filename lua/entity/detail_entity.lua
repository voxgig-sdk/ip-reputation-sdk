...
local DetailEntity = {}
DetailEntity.__index = DetailEntity


function DetailEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DetailEntity)
  self._name = "detail"
  self._client = client
...
function DetailEntity:get_name()
  return self._name
end


function DetailEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DetailEntity.new(self._client, opts)
end
...
function DetailEntity:mark_deleted()
  self._deleted = true
end


function DetailEntity:deleted()
  return true == self._deleted
end


function DetailEntity:data_set(args)
  if args ~= nil then
...
function DetailEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DetailEntity:match_set(args)
  if args ~= nil then
...
function DetailEntity:match_get()
...
function DetailEntity:stream(action, args, callopts)
...
---@param reqmatch DetailLoadMatch
---@param ctrl? table
---@return Detail
---@return string? err
function DetailEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DetailEntity:_run_op(ctx, post_done)
  local utility = self._utility
