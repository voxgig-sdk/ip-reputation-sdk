...
from ipreputation_sdk.utility import register

# Load features
from ipreputation_sdk.feature.base_feature import IpReputationBaseFeature
from ipreputation_sdk.features import _has_feature, _make_feature
...
class IpReputationSDK:
    ...
    def get_utility(self):
        return IpReputationUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("IpReputationSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Detail(self, data=None) -> "DetailEntity":
        """Entity factory: client.Detail().list() / client.Detail().load({"id": ...})."""
        from ipreputation_sdk.entity.detail_entity import DetailEntity
        return DetailEntity(self, data)


    def IpReputation(self, data=None) -> "IpReputationEntity":
        """Entity factory: client.IpReputation().list() / client.IpReputation().load({"id": ...})."""
        from ipreputation_sdk.entity.ip_reputation_entity import IpReputationEntity
        return IpReputationEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "IpReputationSDK":
        if sdkopts is None:
            sdkopts = {}
