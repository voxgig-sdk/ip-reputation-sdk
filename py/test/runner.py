...
class IpReputationTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpReputationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpReputationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpReputationTestRunner.getenv("IP_REPUTATION_TEST_LIVE")
        override = IpReputationTestRunner.getenv("IP_REPUTATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpReputationTestRunner.getenv(key)
        ...
        explain = IpReputationTestRunner.getenv("IP_REPUTATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if IpReputationTestRunner._test_control is not None:
            return IpReputationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpReputationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpReputationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpReputationTestRunner.load_test_control()
...
def load_env_local():
    IpReputationTestRunner.load_env_local()


def env_override(m):
    return IpReputationTestRunner.env_override(m)


def entity_data(v):
    return IpReputationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpReputationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpReputationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpReputationTestRunner.load_test_control()


def live_client_options():
    return IpReputationTestRunner.live_client_options()


def live_delay_ms():
    return IpReputationTestRunner.live_delay_ms()
