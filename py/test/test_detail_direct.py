...
class TestDetailDirect:

    def test_should_direct_load_detail(self):
        setup = _detail_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-detail", "live" if setup["live"] else "unit")
        if _skip:
...
def _detail_direct_setup(mockres):
