...
describe('DetailEntity', async () => {

  // Per-test live pacing. Delay is read from sdk-test-control.json's
  // `test.live.delayMs`; only sleeps when IPREPUTATION_TEST_LIVE=TRUE.
  afterEach(liveDelay('IPREPUTATION_TEST_LIVE'))
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'detail.' + op, live)) return
    }
    ...
    let detail_ref01_data = Object.values(setup.data.existing.detail)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/detail/DetailTestData.json')
  ...
  const idmapEnvVal = process.env['IP_REPUTATION_TEST_DETAIL_ENTID']
