...
const __1 = require("..");
const SDK_NAME = 'IpReputationSDK';
...
const TEST_SEED = { "entity": { "detail": { "test01": { "id": "test01" } }, "ip_reputation": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+IpReputationSDK\b/.test(code) ||
        /\bIpReputationSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+IpReputationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/IpReputationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
