// Command IpReputation-cli is a CLI + REPL for the IpReputation SDK,
...
const prompt = "ip-reputation"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "detail ip_reputation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IP_REPUTATION_APIKEY (api key), IP_REPUTATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IP_REPUTATION_APIKEY carries the API key and
	// IP_REPUTATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IP_REPUTATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IP_REPUTATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpReputationSDK(opts)
