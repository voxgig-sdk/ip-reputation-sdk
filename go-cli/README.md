...
boru-driven command-line client **and** interactive REPL for the IpReputation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-reputation-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-reputation-cli --help

# 3. Provide credentials once, via the environment
export IP_REPUTATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ip-reputation-cli load 1 detail            # {id:1} shorthand
./ip-reputation-cli load '{id:1}' detail       # explicit match map

# 5. Override the API base URL for a single call
IP_REPUTATION_BASE=https://api.example.com ./ip-reputation-cli load 1 detail

# 6. No arguments -> interactive REPL
./ip-reputation-cli
ip-reputation> load 1 detail
ip-reputation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-reputation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-reputation-cli load 1 detail
   ```
...
```sh
./ip-reputation-cli load 1 detail          # scalar shorthand for {id:1}
./ip-reputation-cli load '{id:1}' detail     # explicit match map
```
...
./ip-reputation-cli load 1 detail
```
...
Run with no arguments to open a REPL (prompt `ip-reputation>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `detail`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-reputation-cli`. |
...
(`./ip-reputation-cli <expr>`) and interactively (the REPL), and expressions compose the
