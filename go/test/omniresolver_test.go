...
import (
	...
	omni "github.com/voxgig-sdk/ip-reputation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpReputationError); is {
				out := map[string]any{
					"name":    "IpReputationError",
...
func omniCtx(arg any, client *sdk.IpReputationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpReputationSDK); is {
			client = live
