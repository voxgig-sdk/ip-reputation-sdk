...
func TestDetailEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.Detail(nil)
		if ent == nil {
			t.Fatal("expected non-nil DetailEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := detailBasicSetup(nil)
		...
		for _, _op := range []string{"load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "detail." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPREPUTATION_TEST_DETAIL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func detailBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "detail", "DetailTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read detail test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse detail test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("IPREPUTATION_TEST_DETAIL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPREPUTATION_TEST_DETAIL_ENTID"])
