...
func TestIpReputationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPREPUTATION_TEST_IP_REPUTATION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func ip_reputationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPREPUTATION_TEST_IP_REPUTATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPREPUTATION_TEST_IP_REPUTATION_ENTID"])
