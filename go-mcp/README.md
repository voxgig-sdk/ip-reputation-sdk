...
[MCP](https://modelcontextprotocol.io) server exposing the IpReputation SDK as
two agent tools — `ip-reputation_list` and `ip-reputation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-reputation-mcp)
make build

# 2. Provide credentials via the environment
export IP_REPUTATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ip-reputation \
  -- /absolute/path/to/ip-reputation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-reputation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ip-reputation_list: first page of records
{ "entity": "detail" }
{ "entity": "detail", "query": { } }

// ip-reputation_load: one record by id
{ "entity": "detail", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-reputation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-reputation \
     -- "$PWD"/dist/*/ip-reputation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-reputation_list` and `ip-reputation_load` tools now appear
   in new sessions. Ask the agent to *"list detail using ip-reputation"*
   and it calls `ip-reputation_list` with `{"entity":"detail"}`.
...
```sh
export IP_REPUTATION_APIKEY=sk_live_xxx            # API key
export IP_REPUTATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ip-reputation_list` tool
...
### Call the `ip-reputation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-reputation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-reputation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_REPUTATION_APIKEY` | API key sent with every request. |
| `IP_REPUTATION_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

detail | ip_reputation

### Smoke test via HTTP (raw JSON-RPC)

```sh
./ip-reputation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-reputation_load","arguments":{"entity":"detail","query":{"id":1}}}}'
```
