...
// IpReputation SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class IpReputationMakeError
{
    public static function call(?IpReputationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IpReputationContext([], null);
        }
        $op = $ctx->op ?? new IpReputationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IpReputationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IpReputationError) ? $err->msg : (string)$err;
        $msg = "IpReputationSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new IpReputationError('', $msg, $ctx);
        ...
        if ($err instanceof IpReputationError) {
