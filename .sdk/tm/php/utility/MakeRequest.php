...
class IpReputationMakeRequest
{
    public static function call(IpReputationContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new IpReputationResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new IpReputationResponse($fetched);
        } else {
