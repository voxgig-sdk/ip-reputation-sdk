...
// IpReputation SDK retry feature
...
class IpReputationRetryFeature extends IpReputationBaseFeature
{
    ...
    public function init(IpReputationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpReputationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpReputationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpReputationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
