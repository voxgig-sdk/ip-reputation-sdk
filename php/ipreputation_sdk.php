...
class IpReputationSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = IpReputationHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = IpReputationHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!IpReputationFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, IpReputationFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return IpReputationUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = IpReputationHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = IpReputationHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = IpReputationHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new IpReputationError($op . "_allow",
                "IpReputationSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = IpReputationHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new IpReputationError("graphql_error",
                "IpReputationSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Detail()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->detail()
    // resolves here too.
    public function Detail($data = null)
    {
        require_once __DIR__ . '/entity/detail_entity.php';
        if ($data === null) {
            if ($this->_detail === null) {
                $this->_detail = new DetailEntity($this, null);
            }
            return $this->_detail;
        }
        return new DetailEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new IpReputationSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
