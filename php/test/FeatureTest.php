...
class FtCtrl extends IpReputationControl
{
...
class FtClient
{
    ...
    private IpReputationUtility $utility;

    public function __construct(IpReputationUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpReputationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpReputationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IpReputationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpReputationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpReputationConfig::make_feature($name);
    ...
    private static function buildUrl(IpReputationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpReputationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpReputationResponse($fetched);
        }
    ...
    private function fail(IpReputationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpReputationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
