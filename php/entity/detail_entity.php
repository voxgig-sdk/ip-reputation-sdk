...
// IpReputation SDK Detail entity
...
class DetailEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DetailEntity($this->_client, $opts);
    }

    /**
     * @param Detail|array $args Detail data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IpReputationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Detail|array The current Detail data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Detail fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IpReputationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Detail fields).
     */
    ...
    /**
     * Load a single Detail.
     *
     * @param DetailLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed DetailLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Detail|array The loaded Detail as an assoc-array at the
     *   SDK boundary; throws IpReputationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IpReputationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
