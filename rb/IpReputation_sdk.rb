...
require_relative 'IpReputation_types'
...
class IpReputationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = IpReputationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IpReputationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IpReputationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    IpReputationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = IpReputationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = IpReputationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IpReputationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = IpReputationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IpReputationError.new(
        "graphql_error", "IpReputationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Detail.list / client.Detail.load({ "id" => ... })
  def Detail(data = nil)
    require_relative 'entity/detail_entity'
    DetailEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = IpReputationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
