# IpReputation Ruby SDK



The Ruby SDK for the IpReputation API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Detail` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
raise a `IpReputationError` (a `StandardError` subclass), so wrap
...
| --- | --- | --- |
| `abuse` | `Hash` | Abuse contact information |
| `abuse_score` | `Float` | Numeric abuse score from 0 to 1. |
| `asn` | `Hash` | Autonomous System Number profile |
...
| `location` | `Hash` | Approximate geolocation information |
...
```
rb/
├── IpReputation_sdk.rb       -- Main SDK module
