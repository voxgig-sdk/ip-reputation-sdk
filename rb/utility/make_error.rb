...
module IpReputationUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = IpReputationContext.new({}, nil)
    end
    op = ctx.op || IpReputationOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || IpReputationResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(IpReputationError) ? err.msg : err.to_s
    ...
    sdk_err = IpReputationError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(IpReputationError)
