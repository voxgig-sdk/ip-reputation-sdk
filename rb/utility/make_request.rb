# IpReputation SDK utility: make_request
...
module IpReputationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpReputationResponse.new({})
    ...
    if fetch_err
      ...
      response = IpReputationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpReputationResponse.new(fetched)
    else
