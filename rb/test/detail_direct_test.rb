...
class DetailDirectTest < Minitest::Test
  def test_direct_load_detail
    setup = detail_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-detail", setup[:live] ? "live" : "unit")
    if _should_skip
...
def detail_direct_setup(mockres)
