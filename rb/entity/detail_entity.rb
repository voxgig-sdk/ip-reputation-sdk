...
class DetailEntity
  ...
  def make
    opts = @_entopts.dup
    DetailEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpReputationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Detail, Hash] the current Detail data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpReputationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Detail fields)
  def match_get
  ...
  #
  # @param reqmatch [DetailLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Detail.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Detail, Hash] the loaded Detail; raises IpReputationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpReputationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
